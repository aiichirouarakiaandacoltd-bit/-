...
def check_ytdlp():
    import shutil as sh
    path = sh.which("yt-dlp")
    if path:
        try:
            r = subprocess.run(["yt-dlp", "--version"], capture_output=True, text=True, timeout=5)
            ver = r.stdout.strip()
            return {"name": "yt-dlp", "ok": True, "detail": f"バージョン {ver}"}
        except Exception as e:
            return {"name": "yt-dlp", "ok": False, "detail": str(e)}
    return {"name": "yt-dlp", "ok": False, "detail": "yt-dlpが見つかりません。pip install yt-dlp を実行してください。"}


def run_preflight(mode="production", skip_input_check=False):
    ...
    if not skip_input_check:
        checks.append(check_input_materials())

    auto_fetch = settings.get("channel", {}).get("auto_fetch", False)
    if auto_fetch:
