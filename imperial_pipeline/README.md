...
### 機械検出が拾う項目（`audit/00_機械検出レポート.md`）

| 節 | 検出内容 |
| --- | --- |
| 1 | 必須見出し・必須列の欠落、未置換のプレースホルダ、CSV行数上限超過 |
| 2 | 禁止表現（使用禁止／根拠追加が必要／留保表現に変更の3分類で候補提示） |
| 3 | 過去動画との類似度（文字2-gram。参考値） |
| 4 | 台本が参照する事実ID・素材IDと台帳の突合、**10Aと10Bの本文一致** |
| 5 | 架空URL、AI生成顔の疑い、敬称の表記ゆれ、**設定値の未入力**、台本の文字数 |
| 6 | 未確認タグの出現数 |
| 7 | ID採番の重複・欠番・書式違反 |
| 8 | 安全素材ライブラリの登録件数 |

「設定値の未入力」は、`bgm_credit` や `contact` が初期値のまま成果物へ出ている状態を指します。
このまま公開するとクレジット義務を果たせないため、最終版生成時にも警告が出ます。
